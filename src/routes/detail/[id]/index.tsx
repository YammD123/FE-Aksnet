...
const useEps = routeLoader$<EpsType>(async ({ params, url }) => {
    ...
    try {
        const res = await fetch(`${BASE_URL}/eps-link/video?name=${params.id}&eps=${eps}`);
        const data = await res.json();
        return data.data as EpsType;
    } catch (error) {
        return {
  ...
  return (
    ...
    <div class="flex md:flex-row flex-col gap-6 items-start px-2 py-8">
        <div class="w-full">
            ...
            src={Array.isArray(epsSignal.value?.video_source)?epsSignal.value?.video_source[indexRosolusi]?.video_url?? "" : ""}
            autoFocus
            allow="autoplay; encrypted-media; fullscreen;"
            allowFullscreen
            ></iframe>
            <div class="flex flex-row gap-2  mt-4">
            {Array.isArray(epsSignal.value?.video_source) && epsSignal.value.video_source.map((video,index) => (
                <div>
                    <Button
                    class={` border-gray-950  py-2 px-4 rounded ${index === indexRosolusi ? "text-orange-300 bg-orange-400/45" : "text-white/80 italic bg-zinc-950 border hover:text-white transition-colors"}`}
                    >
                        <Link
        ...
        <div class="text-white grid lg:grid-cols-8 md:grid-cols-6 grid-cols-4 xl:grid-cols-9 border rounded-2xl border-gray-900 p-7 gap-y-2 gap-x-3 ">
            {epsLinkSignal.value.map((eps)=>(
                <Link
                href={`/detail/${id}?eps=${eps.episode_number}`}
                >
                <Button
    ...
    <DetailAnime
    isFavorite={favoriteSignal.value?{ id: favoriteSignal.value.id } : null} 
